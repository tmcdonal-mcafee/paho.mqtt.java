...
public class BasicTest {
	...
	public void testHAConnect() throws Exception {
		...
		try {
			try {
				...
				// If a client does not support the URI list in the connect options, then this
				// test should fail.
	 ...
	 */
	@Test
	public void testCleanDisconnectThreads() throws Exception {
		String methodName = Utility.getMethodName();
		LoggingUtilities.banner(log, cclass, methodName);
		...
		try {
			...
			Assert.assertEquals(5, clientThreads.size());
			...
			log.info(String.format("Immediately after  Disconnect, there are %d client threads running. ",
					clientThreadsAfterDisconnect.size()));
	 ...
	 */
	@Test
	public void testCleanForciblyDisconnectThreads() throws Exception {
		String methodName = Utility.getMethodName();
		LoggingUtilities.banner(log, cclass, methodName);
